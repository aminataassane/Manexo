...
<header
    class="flex sticky z-50 h-14 border-[#E5E7EB] border-b px-4 sm:px-6 top-0 backdrop-blur-md items-center justify-between w-full overflow-x-hidden"
...
>
    ...
    <div class="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
        ...
        <div class="relative flex-1 min-w-0 max-w-sm group hidden sm:block">
            ...
            <input
                type="text"
                placeholder="{{ __('Rechercher un ticket (ID, sujet)...') }}"
    ...
    <div class="flex items-center gap-2 sm:gap-3 pl-3 sm:pl-4 shrink-0">
        <button class="relative h-8 w-8 flex items-center justify-center text-[#6B7280] hover:text-[#111827] hover:bg-white/60 rounded-full transition-colors" title="{{ __('Notifications') }}">
        ...
        <div class="flex items-center gap-1.5 hover:bg-white/60 rounded-full pl-1 pr-2 py-1 transition-colors min-w-0">
            <a
                href="{{ route('profile') }}"
            ...
            >
                ...
                <div class="hidden sm:block text-left">
                    ...
                    <p class="text-[10px] text-[#6B7280] leading-none mt-0.5">{{ $roleLabel }}</p>
                </div>
            </a>

            <form method="POST" action="{{ route('logout') }}" class="ml-1">
                @csrf
                <button type="submit" class="text-[#6B7280] hover:text-red-600 transition-colors cursor-pointer" title="{{ __('Déconnexion') }}">
