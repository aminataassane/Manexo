...
<head>
    ...
    <title>403 – {{ __('Accès refusé') }}</title>
...
<body class="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-4 py-8" style="--accent: {{ request()->attributes->get('currentOrganization')?->primary_color ?? env('MANEXO_DEFAULT_ACCENT', '#005F02') }};">
    <div class="w-full max-w-md text-center">
        ...
        <h1 class="mt-6 text-4xl font-bold tracking-tight text-slate-900">
            ...
            <span>{{ __('Accès refusé') }}</span>
        </h1>
        <p class="mt-3 text-sm leading-relaxed text-slate-600">
            {{ __('Vous n\'avez pas les droits nécessaires pour accéder à cette page ou à cette action.') }}
        </p>

        {{-- Message optionnel de l'exception --}}
        @if($exception->getMessage() && $exception->getMessage() !== 'Forbidden' && $exception->getMessage() !== 'This action is unauthorized.')
