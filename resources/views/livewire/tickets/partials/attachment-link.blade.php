...
@if($isImage && $url !== '#')
    {{-- ══════ IMAGE TILE (used inside grid) ══════ --}}
    <div class="att-image-tile group/img relative cursor-pointer overflow-hidden rounded-xl {{ $variant === 'mine' ? 'border border-[color:color-mix(in_srgb,var(--accent)_18%,#e2e8f0)] bg-[color-mix(in_srgb,var(--accent-soft)_45%,white)]' : ($variant === 'note' ? 'border border-amber-200/50 bg-amber-50/40' : 'border border-slate-200/55 bg-slate-50') }}"
...
@else
    {{-- Fichier : intégré à la bulle (pas d’encadré « technique ») --}}
    <div class="attachment-file-row flex max-w-full cursor-default items-center gap-2.5 rounded-2xl px-2.5 py-2 transition-colors duration-200 sm:gap-3 sm:px-3 sm:py-2.5 {{ $fileRowClass }} group/file">
        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ring-1 {{ $iconWrapClass }}">
            <iconify-icon icon="{{ $icon }}" width="18" class="opacity-90"></iconify-icon>
        </div>
        <div class="min-w-0 flex-1 py-0.5">
            <p class="truncate text-[13px] font-medium leading-snug tracking-tight {{ $nameClass }}">{{ $name }}</p>
            <p class="mt-0.5 text-[11px] font-normal tabular-nums {{ $metaClass }}">
                @if($extLabel)<span class="uppercase tracking-wide">{{ $extLabel }}</span>@endif
                @if($size)<span>@if($extLabel)<span class="mx-1 opacity-40">·</span>@endif{{ $size }}</span>@endif
            </p>
        </div>
        <div class="flex shrink-0 items-center gap-0.5 self-center rounded-full p-0.5 shadow-sm ring-1 backdrop-blur-[2px] transition-opacity duration-200 opacity-80 group-hover/file:opacity-100 {{ $actionPillClass }}">
            @if($isPdf)
                <button type="button"
                        @click="$dispatch('open-attachment-preview', { url: '{{ $url }}', name: '{{ e($name) }}', type: 'pdf' })"
                        title="{{ __('Aperçu') }}"
