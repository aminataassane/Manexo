    ...
    @if($item->isOwn)
        <div id="message-{{ $item->id }}" class="message-row message-own flex justify-end py-2 sm:py-3 min-w-0">
            <div class="flex items-end gap-2 sm:gap-3 max-w-[95%] sm:max-w-[85%] min-w-0 flex-row-reverse">
                @if($item->showAvatarPopover)
                    <x-manexo.user-avatar-popover
                        :initials="$item->initials"
                        :name="$item->authorName ?? '—'"
                        :email="$item->authorEmail"
                        :mention-tag="$item->mentionTag"
                        :badges="$item->popoverBadges"
                        :open-label="__('tickets.timeline_user_card_open')"
                        size-class="min-h-11 min-w-11 h-11 w-11 sm:h-9 sm:w-9 sm:min-h-0 sm:min-w-0 ring-2 ring-white shadow-md"
                    />
                @else
                ...
                <div class="flex flex-col items-end min-w-0 max-w-full">
                    <div class="mb-1 flex w-full min-w-0 flex-col items-stretch gap-1 sm:mb-1.5 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end sm:gap-x-2 sm:gap-y-1">
                        <div class="flex min-w-0 flex-wrap items-center justify-end gap-1.5 sm:justify-end">
                            <span class="max-w-full text-[11px] font-semibold leading-snug text-slate-800 [overflow-wrap:anywhere] sm:text-xs">{{ $item->authorName ?? '—' }}</span>
                            ...
                            @if($item->hasEmailOrigin)
                                <span class="inline-flex max-w-full min-w-0 shrink items-center gap-1 rounded-md border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[9px] font-medium text-emerald-600 sm:text-[10px]">
                                    ...
                                    <span class="min-w-0 truncate sm:whitespace-normal sm:[overflow-wrap:anywhere]">{{ $item->badgePrimary }}</span>
                                </span>
                            ...
                            @else
                                <span class="inline-flex max-w-full rounded-md bg-slate-700/10 px-2 py-0.5 text-[9px] font-medium text-slate-600 sm:text-[10px]">{{ $item->roleLabel }}</span>
                            @endif
                        </div>
                        <span class="text-end text-[10px] text-slate-400 sm:ml-auto sm:text-start">{{ $item->timeHuman }}</span>
                    </div>
            ...
            <div class="min-w-0 w-full max-w-full sm:w-fit">
                <div class="mb-1 flex w-full min-w-0 flex-col gap-1 sm:mb-1.5 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-2 sm:gap-y-1">
                    <div class="flex min-w-0 flex-wrap items-center gap-1.5">
                        ...
                        @if($item->hasEmailOrigin)
                            <span class="inline-flex max-w-full min-w-0 shrink items-center gap-1 rounded-md px-2 py-0.5 text-[9px] font-medium sm:text-[10px] {{ $item->channel === 'email_inbound' ? 'border border-blue-200 bg-blue-50 text-blue-600' : 'border border-emerald-200 bg-emerald-50 text-emerald-600' }}">
                                <iconify-icon icon="solar:letter-linear" width="10" class="shrink-0"></iconify-icon>
                                <span class="min-w-0 truncate sm:whitespace-normal sm:[overflow-wrap:anywhere]">{{ $item->badgePrimary }}</span>
                            </span>
                        @elseif($item->channel === 'api')
                        ...
                        @else
                            <span class="inline-flex max-w-full rounded-md border border-slate-200/80 bg-slate-100 px-2 py-0.5 text-[9px] font-medium text-slate-600 sm:text-[10px]">{{ $item->roleLabel }}</span>
                        @endif
                    </div>
                    <span class="text-[10px] text-slate-400 sm:ml-auto">{{ $item->timeHuman }}</span>
                </div>

                @if($item->hasEmailOrigin && $item->emailFrom)
                    <div class="mb-1 text-[10px] leading-snug text-slate-400 [overflow-wrap:anywhere] break-all">via {{ $item->emailFrom }}</div>
                @endif
