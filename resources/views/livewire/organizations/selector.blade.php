        ...
        @if($isSwitching)
            ...
            <div class="w-full max-w-2xl fade-in">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    @forelse ($organizations as $org)
                        ...
                        <button
                            ...
                            class="group relative flex items-center gap-4 rounded-2xl border p-4 text-left transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 {{ $isActive ? 'border-[var(--accent)] bg-[var(--accent-soft)]/10 ring-1 ring-[var(--accent)]' : 'border-slate-200 bg-white hover:border-[var(--accent-soft)]' }}"
                        >
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="font-semibold text-slate-900 truncate group-hover:text-[var(--accent)] transition-colors">{{ $org->name }}</div>
                                <div class="text-xs text-slate-500 flex items-center gap-1.5 mt-0.5">
                                    <span class="inline-block w-1.5 h-1.5 rounded-full {{ $isActive ? 'bg-emerald-500' : 'bg-slate-300' }}"></span>
                                    {{ $roleLabel }}
                                </div>
                            </div>
                            @if($isActive)
                ...
                <div class="mt-10 text-center">
                    <a href="{{ route('dashboard') }}" class="text-sm font-medium text-slate-500 hover:text-slate-900 transition-colors inline-flex items-center gap-2">
