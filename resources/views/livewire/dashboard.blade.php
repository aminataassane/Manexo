...
@php
    ...
    $myKpis = $this->myKpis;
    $myRecentTickets = $this->myRecentTickets;
@endphp
...
@else
    ...
    <div class="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4 lg:gap-6">
        <div class="group relative overflow-hidden rounded-xl sm:rounded-2xl bg-white p-4 sm:p-5 shadow-sm border border-slate-100 hover:shadow-md transition-all duration-300 min-w-0">
            <div class="flex items-center justify-between gap-2">
                <div class="min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-slate-500 truncate">{{ __('pages.dashboard.my_open_tickets') }}</p>
                    <p class="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold text-slate-900">{{ $myKpis['my_open'] }}</p>
                </div>
        ...
        <div class="group relative overflow-hidden rounded-xl sm:rounded-2xl bg-white p-4 sm:p-5 shadow-sm border border-slate-100 hover:shadow-md transition-all duration-300 min-w-0">
            <div class="flex items-center justify-between gap-2">
                <div class="min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-slate-500 truncate">{{ __('pages.dashboard.my_in_progress') }}</p>
                    <p class="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold text-slate-900">{{ $myKpis['my_in_progress'] }}</p>
                </div>
        ...
        <div class="group relative overflow-hidden rounded-xl sm:rounded-2xl bg-white p-4 sm:p-5 shadow-sm border border-slate-100 hover:shadow-md transition-all duration-300 min-w-0">
            <div class="flex items-center justify-between gap-2">
                <div class="min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-slate-500 truncate">{{ __('pages.dashboard.my_total_tickets') }}</p>
                    <p class="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold text-slate-900">{{ $myKpis['my_total'] }}</p>
                </div>
        ...
        <div class="group relative overflow-hidden rounded-xl sm:rounded-2xl bg-white p-4 sm:p-5 shadow-sm border border-slate-100 hover:shadow-md transition-all duration-300 min-w-0">
            <div class="flex items-center justify-between gap-2">
                <div class="min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-slate-500 truncate">{{ __('pages.dashboard.my_resolved') }}</p>
                    <p class="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold text-slate-900">{{ $myKpis['my_resolved'] }}</p>
                </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-3">
        <div class="space-y-4 sm:space-y-6 lg:col-span-2 min-w-0">
            ...
            <div class="rounded-xl sm:rounded-2xl bg-white shadow-sm border border-slate-100 overflow-hidden min-w-0">
                <div class="px-4 sm:px-6 py-4 sm:py-5 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <a href="{{ route('tickets.index') }}" class="text-sm font-semibold hover:underline" style="color: var(--accent);">{{ __('pages.dashboard.see_all') }}</a>
                </div>
                <div class="responsive-table-wrap">
        ...
        <div class="space-y-4 sm:space-y-6 min-w-0">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <div class="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-0.5 text-xs text-slate-500">
                                            ...
                                            @if($fa->due_date)
                                                <span class="flex items-center gap-1">
                                                    <iconify-icon icon="solar:calendar-linear" width="12"></iconify-icon>
                                                    {{ $fa->due_date->format('d/m/Y') }}
                                                </span>
                                            @endif
                                        </div>
                                    </div>
                                    <iconify-icon icon="solar:arrow-right-linear" width="16" class="text-slate-400 group-hover:text-[var(--accent)] transition-colors shrink-0"></iconify-icon>
                                </a>
                            </li>
                        @endforeach
                    </ul>
            ...
            <div class="rounded-xl sm:rounded-2xl bg-white p-4 sm:p-6 shadow-sm border border-slate-100">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-base sm:text-lg font-bold text-slate-900">{{ __('pages.dashboard.discussions') }}</h3>
                    @if ($discussionsUnreadCount > 0)
                        <span class="flex h-6 min-w-[24px] px-1.5 items-center justify-center rounded-full bg-red-100 text-xs font-bold text-red-600">{{ $discussionsUnreadCount }}</span>
                    @endif
                </div>
                <div class="space-y-3">
                    @forelse ($recentDiscussions as $d)
                        <a href="{{ $d->url }}" class="flex gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors group block">
                            <div class="h-9 w-9 rounded-full flex items-center justify-center text-xs font-bold ring-2 ring-white shrink-0" style="background: var(--accent-soft); color: var(--accent);">{{ strtoupper(mb_substr($d->name ?: 'D', 0, 1)) }}</div>
                            <div class="min-w-0 flex-1">
                                <div class="flex items-center justify-between gap-2 mb-0.5">
                                    <p class="text-sm font-bold text-slate-900 truncate">{{ $d->name ?: __('pages.dashboard.discussions') }}</p>
                                    <span class="text-[10px] text-slate-400 shrink-0">{{ $d->last_at?->diffForHumans() ?: '—' }}</span>
                                </div>
                                <p class="text-xs text-slate-500 line-clamp-1 group-hover:text-slate-700">{{ $d->last_body ?: __('pages.dashboard.no_message') }}</p>
                            </div>
                        </a>
                    @empty
                        <p class="text-sm text-slate-500 py-4 text-center">{{ __('pages.dashboard.no_recent_discussion') }}</p>
                ...
                <a href="{{ route('discussions.index') }}" class="mt-4 w-full flex rounded-xl border border-slate-200 bg-white py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors justify-center">
                    {{ __('pages.dashboard.see_all_discussions') }}
                </a>
            </div>
            ...
            <!-- RECENT ACTIVITY -->
            <div class="rounded-xl sm:rounded-2xl bg-white p-4 sm:p-6 shadow-sm border border-slate-100">
                <h3 class="text-base sm:text-lg font-bold text-slate-900 mb-4">{{ __('pages.dashboard.recent_activity') }}</h3>
                <div class="relative pl-4 space-y-5 before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
                    @forelse ($recentActivity as $a)
                        <a href="{{ $a->url }}" class="relative pl-6 block group">
                            <div class="absolute left-0 top-1.5 h-2.5 w-2.5 rounded-full border-2 border-white shadow-sm group-hover:scale-110 transition-transform" style="background-color: var(--accent);"></div>
                            <p class="text-sm font-medium text-slate-900 group-hover:text-[var(--accent)] truncate">{{ $a->subject }}</p>
                            <p class="text-xs text-slate-500 mt-0.5">{{ $a->created_at->diffForHumans() }} {{ __('pages.dashboard.by') }} <span class="font-medium text-slate-700">{{ $a->user_name }}</span></p>
                        </a>
                    @empty
                        <p class="text-sm text-slate-500 py-2">{{ __('pages.dashboard.no_recent_activity') }}</p>
