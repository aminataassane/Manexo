...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($item->isSystem): ?>
    <div id="message-<?php echo e($item->id); ?>" class="message-row message-system flex justify-center py-3 min-w-0">
        <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-100 text-slate-600 text-xs">
            ...
            <span class="break-words max-w-[min(100%,28rem)]"><?php echo $item->body; ?></span>
            <span class="text-slate-400 shrink-0">· <?php echo e($item->timeHuman); ?></span>
        </div>
    </div>
<?php elseif($item->isInternal): ?>
    <div id="message-<?php echo e($item->id); ?>" class="message-row message-note flex gap-3 py-3 min-w-0 max-w-[85%]">
        ...
        <div class="message-bubble flex-1 min-w-0 rounded-2xl rounded-tl-md bg-amber-50/90 border border-amber-200/80 shadow-sm overflow-hidden">
            <div class="px-4 py-2.5 flex flex-wrap items-center justify-between gap-2 border-b border-amber-200/60 bg-amber-50/50">
                <div class="flex items-center gap-2">
                    <span class="text-[11px] font-bold text-amber-800 uppercase tracking-wide"><?php echo e(__('Note interne')); ?></span>
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($item->hasAttachments): ?>
                <div class="px-4 py-3 pt-2 border-t border-amber-200/50 space-y-2 flex flex-wrap gap-2">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $item->attachments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $att): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                        <?php echo $__env->make('livewire.tickets.partials.attachment-link', ['att' => $att, 'variant' => 'note'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
...
<?php else: ?>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($item->isOwn): ?>
        <div id="message-<?php echo e($item->id); ?>" class="message-row message-own flex justify-end py-3 min-w-0">
            <div class="flex items-end gap-3 max-w-[85%] min-w-0 flex-row-reverse">
                ...
                <div class="flex flex-col items-end min-w-0 max-w-full">
                    <div class="flex items-center gap-2 mb-1.5 flex-row-reverse">
                        <span class="text-xs font-semibold text-slate-800"><?php echo e($item->authorName ?? '—'); ?></span>
                        ...
                        <?php else: ?>
                            <span class="px-2 py-0.5 rounded-md bg-slate-700/10 text-[10px] font-medium text-slate-600"><?php echo e($item->roleLabel); ?></span>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>

                        <span class="text-[10px] text-slate-400"><?php echo e($item->timeHuman); ?></span>
                    </div>
                    <div class="message-bubble rounded-2xl rounded-br-md px-4 py-3 text-sm leading-relaxed shadow-md w-full max-w-full <?php echo e($item->hasAttachments ? 'bg-white border border-slate-200 text-slate-700' : ''); ?>" <?php if(!$item->hasAttachments): ?> style="background: linear-gradient(135deg, var(--accent) 0%, color-mix(in srgb, var(--accent) 85%, #1e293b) 100%); color: #fff;" <?php endif; ?>>
                        <div class="text-left break-words <?php echo e(!$item->hasAttachments ? 'text-white' : ''); ?>">
                            <?php echo $item->body; ?>
    ...
    <?php else: ?>
        <div id="message-<?php echo e($item->id); ?>" class="message-row message-incoming flex gap-3 py-3 min-w-0 max-w-[85%]">
            <div class="w-9 h-9 shrink-0 rounded-full ring-2 ring-white shadow-md overflow-hidden bg-slate-100">
                <img src="<?php echo e($item->avatarUrl); ?>" class="w-full h-full object-cover" alt="">
            </div>
            <div class="min-w-0 max-w-full w-fit">
                <div class="flex flex-wrap items-center gap-2 mb-1.5">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($item->hasEmailOrigin): ?>
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-medium <?php echo e($item->channel === 'email_inbound' ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'bg-emerald-50 text-emerald-600 border border-emerald-200'); ?>">
                            <iconify-icon icon="solar:letter-linear" width="10"></iconify-icon>
                            <?php echo e($item->badgePrimary); ?>
                    ...
                    <?php else: ?>
                        <span class="px-2 py-0.5 rounded-md bg-slate-100 text-[10px] font-medium text-slate-600 border border-slate-200/80"><?php echo e($item->roleLabel); ?></span>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
