<!DOCTYPE html>
<html lang="<?php echo e(str_replace('_', '-', app()->getLocale())); ?>" class="h-full bg-slate-50 antialiased selection:bg-[#F2E3BB] selection:text-[#005F02]">
<head>
    <meta charset="utf-8">
...
<body class="overflow-x-hidden flex items-center justify-center bg-slate-50 w-full min-h-screen min-h-[100dvh] relative py-6 px-4 sm:py-8" style="padding-left: max(1rem, env(safe-area-inset-left)); padding-right: max(1rem, env(safe-area-inset-right)); padding-top: max(1.5rem, env(safe-area-inset-top)); padding-bottom: max(1.5rem, env(safe-area-inset-bottom));">
    ...
    <div class="relative z-10 w-full min-w-0 max-w-[420px] sm:max-w-[440px] min-[1920px]:max-w-[480px] px-0 sm:px-4">
        ...
        <div class="fixed z-50 fade-in top-4 left-4 sm:top-6 sm:left-6 min-[1920px]:top-8 min-[1920px]:left-8" style="animation-delay: 0.1s; top: max(1rem, env(safe-area-inset-top)); left: max(1rem, env(safe-area-inset-left));">
            <a href="<?php echo e(route('home')); ?>" class="group inline-flex items-center gap-2 px-3 py-2.5 sm:py-1.5 min-h-[44px] sm:min-h-0 rounded-full bg-white/50 border border-white/60 hover:bg-white hover:border-slate-200 transition-all duration-200 shadow-sm backdrop-blur-sm">
                ...
                <span class="text-[11px] font-medium text-slate-600 group-hover:text-slate-900"><?php echo e(__('Back')); ?></span>
            </a>
        </div>
        ...
        <div class="fixed z-50 fade-in top-4 right-4 sm:top-6 sm:right-6 min-[1920px]:top-8 min-[1920px]:right-8 flex items-center gap-1 rounded-full bg-white/50 border border-white/60 px-2 py-1.5 shadow-sm backdrop-blur-sm" style="top: max(1rem, env(safe-area-inset-top)); right: max(1rem, env(safe-area-inset-right));">
            <a href="<?php echo e(route('locale.switch', 'fr')); ?>" class="rounded-full px-2.5 py-1 text-[11px] font-semibold transition-colors <?php echo e(app()->getLocale() === 'fr' ? 'bg-[#005F02]/15 text-[#005F02]' : 'text-slate-500 hover:text-slate-800'); ?>"><?php echo e(__('French')); ?></a>
            <span class="text-slate-300">|</span>
            <a href="<?php echo e(route('locale.switch', 'en')); ?>" class="rounded-full px-2.5 py-1 text-[11px] font-semibold transition-colors <?php echo e(app()->getLocale() === 'en' ? 'bg-[#005F02]/15 text-[#005F02]' : 'text-slate-500 hover:text-slate-800'); ?>"><?php echo e(__('English')); ?></a>
        </div>

        <?php echo e($slot); ?>


        <p class="mt-8 text-center text-[10px] text-slate-400 tracking-wide font-medium">© <?php echo e(date('Y')); ?> <?php echo e(config('app.name', 'Manexo')); ?> INC.</p>
    </div>
...
</html><?php /**PATH C:\Users\Aminata_an\OneDrive\Bureau\QUALITY_CENTER\Manexo\manexo\resources\views\layouts\guest.blade.php ENDPATH**/ ?>