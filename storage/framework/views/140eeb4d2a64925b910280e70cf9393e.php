...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($showImagePreview): ?>
    <div class="inline-block max-w-[220px]">
        <a href="<?php echo e($url); ?>" target="_blank" rel="noopener" class="block rounded-xl overflow-hidden border border-slate-200/80 shadow-sm transition-shadow hover:shadow-md">
            <img src="<?php echo e($url); ?>" alt="<?php echo e($name); ?>" class="block w-full h-auto max-h-40 object-contain object-center" loading="lazy">
        </a>
        <a href="<?php echo e($url); ?>" target="_blank" rel="noopener" class="mt-1.5 flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-800 transition-colors">
            <span class="truncate font-medium"><?php echo e($name); ?></span>
            <iconify-icon icon="solar:download-linear" class="shrink-0 opacity-70" width="12"></iconify-icon>
        </a>
    </div>
<?php else: ?>
    <a href="<?php echo e($url); ?>" target="_blank" rel="noopener" class="inline-flex items-center gap-2.5 min-w-0 rounded-xl border border-slate-200/80 bg-slate-50/50 hover:bg-slate-50 p-2.5 transition-colors max-w-[220px] text-slate-700 shadow-sm">
        <div class="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 bg-white text-slate-500 border border-slate-200/60">
            <iconify-icon icon="<?php echo e($icon); ?>" width="16"></iconify-icon>
        </div>
        <div class="min-w-0 flex-1 text-left">
            <p class="text-xs font-medium truncate leading-tight"><?php echo e($name); ?></p>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($size): ?>
                <p class="text-[10px] text-slate-500 mt-0.5"><?php echo e($size); ?></p>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        </div>
        <iconify-icon icon="solar:download-linear" class="shrink-0 text-slate-400" width="14"></iconify-icon>
    </a>
<?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
<?php /**PATH C:\Users\Aminata_an\OneDrive\Bureau\QUALITY_CENTER\Manexo\manexo\resources\views/livewire/tickets/partials/attachment-link.blade.php ENDPATH**/ ?>